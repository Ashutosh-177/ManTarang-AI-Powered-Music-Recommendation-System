...
class LastFmClient(BaseAPIClient):
    ...
    async def search_by_tags(
        ...
        Args:
            ...
            limit: Maximum results
            page: Page number to fetch (random 1-5 if not specified, for variety)
        ...
        for tag in tags:
            try:
                # Use a random page each call so we don't always get the same top tracks
                tag_page = page if page is not None else random.randint(1, 5)
                data = await self._make_lastfm_request(
                    "tag.getTopTracks",
                    {
                        "tag": tag,
