...
class GenreMoodAgent(BaseAgent):
    ...
    async def _create_recommendations(
        ...
        # Generate reasoning for all candidates in a single batch LLM call
        reasoning_list = await self._generate_batch_reasoning(candidates, entities, intent_analysis)
        ...
        for i, candidate in enumerate(candidates):
            try:
                reasoning = (
                    ...
                    else self._create_fallback_reasoning(candidate, entities, intent_analysis, i + 1)
                )
    ...
    async def _generate_batch_reasoning(
        self,
        candidates: List[Dict[str, Any]],
        entities: Dict[str, Any],
        intent_analysis: Dict[str, Any]
        ...
        """
        Generate reasoning for all candidates in a single LLM call to stay within rate limits.
        Falls back to rule-based reasoning if LLM is unavailable.
        """
        if not self.llm_utils or not candidates:
            return [
                self._create_fallback_reasoning(c, entities, intent_analysis, i + 1)
                for i, c in enumerate(candidates)
            ]

        try:
            intent = intent_analysis.get('intent', 'genre_mood')
            genres_requested = entities.get('genres', [])
            moods_requested = entities.get('moods', [])

            prompt_parts = [
                "Generate brief, engaging explanations for why each track matches the listener's genre and mood request.",
                f"User Intent: {intent}",
            ]
            if genres_requested:
                prompt_parts.append(f"Requested Genres: {', '.join(genres_requested[:3])}")
            if moods_requested:
                prompt_parts.append(f"Requested Moods: {', '.join(moods_requested[:3])}")

            prompt_parts.extend([
                "",
                "For each track, provide a concise 1-2 sentence explanation focused on genre/mood fit.",
                "",
                "Tracks:"
            ])
            ...
            for i, candidate in enumerate(candidates, 1):
                ...
                genre_score = candidate.get('genre_score', 0)
                mood_score = candidate.get('mood_score', 0)
                prompt_parts.extend([
                    f"{i}. {name} by {artist}",
                    f"   Tags: {', '.join(tags) if tags else 'none'}",
                    f"   Genre fit: {genre_score:.2f}, Mood fit: {mood_score:.2f}",
                    ""
                ])
            ...
            prompt_parts.extend([
                ...
                "2. [Explanation]",
                "...",
                "Keep each explanation concise and focused on genre/mood match."
            ])

            response = await self.llm_utils.call_llm("\n".join(prompt_parts))
            return self._parse_numbered_response(response, len(candidates))

        except Exception as e:
            self.logger.warning(f"Batch LLM reasoning failed, using fallback: {e}")
            return [
                self._create_fallback_reasoning(c, entities, intent_analysis, i + 1)
                for i, c in enumerate(candidates)
            ]
    ...
    def _parse_numbered_response(self, response: str, expected_count: int) -> List[str]:
        """Parse a numbered list response from the LLM into individual strings."""
        import re
        ...
        while len(results) < expected_count:
            results.append("A strong genre and mood match for your request.")
    ...
    def _create_fallback_reasoning(
        ...
        """Create rule-based reasoning when LLM is unavailable."""
